...
export default async function handler(request, res) {
  let { stations, lines } = request.body;
  // goal: L in 2, 5 minutes
  let updates = {};
  let messages = [];
  try {
    for (let i = 0; i < lines.length; i++) {
      let lineURL = getRelevantURL(lines[i]);
      let lineData = await getLineData(lineURL, stations, lines[i]);
      updates[lines[i]] = lineData;
    }

    // let message = buildMessage(updates.);
    // i guess build messages now
    Object.keys(updates).forEach((line) => {
      let message = buildMessage(updates[line].updates);
      messages.push(message);
    });
    // res.status(200).json({ messages: messages, updates: updates });
...
let getLineData = async (url, stations, line) => {
  try {
    const response = await fetch(url, {
      headers: {
        "x-api-key": "hM3jvw579ENh0VsuiXId9aim7WgoOZL2LGK4yOab",
      },
    });
    if (!response.ok) {
    ...
    feed.entity.forEach((entity) => {
      ...
      let { stopTimeUpdate } = entity.tripUpdate;

      if (stopTimeUpdate && stopTimeUpdate.length > 0 !== null) {
        stopTimeUpdate.forEach((update) => {
          if (stations.includes(update.stopId)) {
            // console.log(update);

            if (update && update.arrival && update.arrival.time) {
              let arrivalTime = dayjs.unix(update.arrival.time);
              let minutesUntilArrival = arrivalTime.diff(now, "minute");

              let currentStationName = stationData.find((i) => {
                return i.stop_id === update.stopId;
              });

              updates.push({
                stopId: update.stopId,
                minutesUntilArrival: minutesUntilArrival,
                arrivalTime: arrivalTime.format("h:mm A"),
                stationName: currentStationName.stop_name,
                trainValue: entity.tripUpdate.trip.routeId,
              });
            } else if (update && update.departure && update.departure.time) {
              let departureTime = dayjs.unix(update.departure.time);
              let minutesUntilDeparture = departureTime.diff(now, "minute");
              ...
              updates.push({
                ...
                arrivalTime: departureTime.format("h:mm A"),
                stationName: currentStationName.stop_name,
                trainValue: entity.tripUpdate.trip.routeId,
              });
            }
...
const buildMessage = (lineUpdates) => {
  let nextArrivals = {};
  lineUpdates.forEach((line) => {
    if (
      nextArrivals[line.trainValue] &&
      nextArrivals[line.trainValue].length < 2
    ) {
      nextArrivals[line.trainValue].push(line.minutesUntilArrival);
    } else if (!nextArrivals[line.trainValue]) {
      nextArrivals[line.trainValue] = [line.minutesUntilArrival];
    }
  });
  ...
  let message = "";
  Object.keys(nextArrivals).forEach((train) => {
    let currentTimes = nextArrivals[train];
    let sortedTimes = currentTimes.sort((a, b) => a - b);
    message += `${train}: ${sortedTimes.join(", ")} minutes\n`;
  });
  return message;
};
